...
@Operation(summary = "Eliminar producto (soft delete)", description = "Marca el producto como eliminado sin borrarlo físicamente. Se guarda en el historial. "
        +
        "<strong>Solo accesible para usuarios con rol ADMIN.</strong>", security = @SecurityRequirement(name = "bearer-key"))
@ApiResponses({
        @ApiResponse(responseCode = "200", description = "Producto eliminado exitosamente", content = @Content(examples = @ExampleObject(value = "{\"success\":true,\"message\":\"Producto eliminado exitosamente\",\"data\":null}"))),

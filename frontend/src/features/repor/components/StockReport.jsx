...
const StockReport = () => {
  ...
  const { chartLabels, chartData } = useMemo(() => {
    ...
    if (isSameDay) {
      ...
      if (startDate && endDate) {
        ...
        dataPoints = rowsArray.map(r => (r.currentStock == null || r.currentStock === '-') ? 0 : r.currentStock);
      } else {
        labels = data.length > 0 ? data.map(item => formatDate(item.date)) : generatePlaceholderLabels(startDate);
        dataPoints = data.length > 0 ? data.map(item => item.currentStock) : Array(labels.length).fill(0);
      }
    }
  ...
  useEffect(() => {
    if (productName && startDate && endDate) {
      ...
      setReportData({
        ...
        chartComponent: <ChartStock dataPoints={chartData} labels={chartLabels} forceHourly={(rowsArray.length === 0) && (!(startDate && endDate) || sameDayForReport)} showPlaceholder={showPlaceholder} />,
  ...
  }, [productName, startDate, endDate, rowsArray, chartData, chartLabels, setReportData]);

  return (
    <>
      <div className="mb-10 mt-4 w-full p-6 bg-white rounded-xl shadow-sm border border-stokia-neutral-200" style={{ height: '350px' }}>
        <ChartStock dataPoints={chartData} labels={chartLabels} forceHourly={rowsArray.length === 0 && (!(startDate && endDate) || isSameDay)} showPlaceholder={!!productName || (startDate && endDate)} />
      </div>

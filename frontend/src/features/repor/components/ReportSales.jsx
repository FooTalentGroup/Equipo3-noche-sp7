  ...
  return (
    <div className="mt-4">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-2xl font-bold text-stokia-neutral-900">
            Reporte de ventas
          </h2>
          <p className="text-stokia-neutral-500 mt-1">
            Analiza los ingresos, la cantidad de ventas y los métodos de pago
            más utilizados en tu negocio.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex-1 max-w-sm">
          <label className="block text-sm font-medium text-stokia-neutral-700 mb-2">
            Producto
          </label>
      ...
      <Dialog open={showCalendar} onOpenChange={setShowCalendar}>
        <DialogContent className="max-h-[90vh] overflow-y-auto p-9 flex flex-col gap-4 max-w-lg bg-white border-0 items-center">
          ...
          <div className="flex gap-9">
            ...
            <Button variant="stokia" onClick={() => setShowCalendar(false)}>
              Aplicar
            </Button>
          </div>
        </DialogContent>
      ...
      {!isLoading && data && (
        <>
          <div className="grid grid-cols-3 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-stokia-neutral-200 p-6 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <span className="text-stokia-neutral-600 text-sm font-medium">
                  Ingreso total por ventas
                </span>
                <span
                  className={`px-2 py-1 rounded text-xs font-medium ${getPercentageColor(
                    data.metrics?.revenueChangePercentage
                  )}`}
                >
                  {data.metrics?.revenueChangePercentage > 0 ? "+" : ""}
                  {data.metrics?.revenueChangePercentage || 0}%
                </span>
              </div>
            ...
            <div className="bg-white rounded-xl border border-stokia-neutral-200 p-6 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-medium ${getPercentageColor(
                    data.metrics?.ordersChangePercentage
                  )}`}
                >
                  {data.metrics?.ordersChangePercentage > 0 ? "+" : ""}
                  {data.metrics?.ordersChangePercentage || 0}%
                </span>
              </div>
              <p className="text-3xl font-bold text-stokia-neutral-900">
            ...
            <div className="bg-white rounded-xl border border-stokia-neutral-200 p-6 shadow-sm">
              <div className="flex justify-between items-start mb-2">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-medium ${getPercentageColor(
                    data.metrics?.averageTicketChangePercentage
                  )}`}
                >
                  {data.metrics?.averageTicketChangePercentage > 0 ? "+" : ""}
                  {data.metrics?.averageTicketChangePercentage || 0}%
                </span>
              </div>
              <p className="text-3xl font-bold text-stokia-neutral-900">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div
              className="col-span-2 bg-white rounded-xl border border-stokia-neutral-200 p-6 shadow-sm"
              style={{ height: "350px" }}
            >
              <ChartSalesDaily dailySales={data.dailySales || []} />
            </div>
            <div
              className="bg-white rounded-xl border border-stokia-neutral-200 p-6 shadow-sm"
              style={{ height: "350px" }}
            >
              <ChartPaymentMethods
                paymentMethods={data.paymentMethodDistribution || []}

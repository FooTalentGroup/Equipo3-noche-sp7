      ...
      <div className="relative w-[760px] max-w-full bg-[#F4F5F7] rounded-2xl shadow-[-3px_2px_4px_0px_rgba(0,0,0,0.25)] p-8 z-10">
        {!isSubmitting && !showSuccess && (
          <form onSubmit={submit} className="flex flex-col">
            <div className="mb-4">
              <h3 className="text-3xl font-semibold text-stokia-neutral-950">{isEditMode ? 'Editar usuario' : 'Registrar nuevo usuario'}</h3>
              <p className="text-xs text-stokia-neutral-500 mt-1">{isEditMode ? 'Actualiza la información del usuario' : 'Completa este formulario para registrar un nuevo usuario'}</p>
            </div>

            <div className="border-t border-Stockia-Neutral-200 mb-6" />

            <div className="space-y-4">
              <div>
                <label className="text-[1rem] font-medium text-stokia-neutral-950 flex items-center gap-1">Nombre completo <span className="text-destructive">*</span></label>
                <Input className={cn(
                  "bg-card",
              ...
              <div>
                <label className="text-[1rem] font-medium text-stokia-neutral-950 flex items-center gap-1">Correo electrónico <span className="text-destructive">*</span></label>
              ...
              {(!isEditMode || changePassword) && (
                <div>
                  <label className="text-[1rem] font-medium text-stokia-neutral-950 flex items-center gap-1">Contraseña <span className="text-destructive">*</span></label>
                  <div className="relative mt-2">
                    <Input type={showPassword ? 'text' : 'password'} className={cn(
                      "bg-card",
                  ...
                  <div className="mt-3">
                    <label className="text-[1rem] font-medium text-stokia-neutral-950 flex items-center gap-1">Repetir contraseña <span className="text-destructive">*</span></label>
                    <div className="relative mt-2">
                      <Input type={showPassword ? 'text' : 'password'} className={
                        cn(
                          "bg-card",
                          errors.repeatPassword && "border-destructive focus-visible:ring-destructive",
                        )
                      } value={form.repeatPassword} onChange={(e) => setForm(f => ({ ...f, repeatPassword: e.target.value }))} />
                      <button type="button" onClick={() => setShowPassword(s => !s)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1">
                        {showPassword ? (
                          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M10 4.58398C11.5678 4.58399 13.1004 5.04984 14.4033 5.92188C15.6247 6.73941 16.5934 7.87935 17.2021 9.21289L17.3184 9.48242L17.3281 9.50684C17.4461 9.82484 17.4462 10.1752 17.3281 10.4932C17.325 10.5015 17.3217 10.5104 17.3184 10.5186C16.7206 11.9677 15.7061 13.2072 14.4033 14.0791C13.1004 14.9511 11.5678 15.416 10 15.416C8.43224 15.416 6.89958 14.9511 5.59668 14.0791C4.29393 13.2072 3.27944 11.9677 2.68164 10.5186C2.67827 10.5104 2.67496 10.5015 2.67188 10.4932C2.55383 10.1752 2.55389 9.82484 2.67188 9.50684C2.67491 9.49869 2.67832 9.49046 2.68164 9.48242C3.27936 8.03311 4.29387 6.79391 5.59668 5.92188C6.89962 5.04984 8.43217 4.58399 10 4.58398ZM10 6.08398C8.72923 6.08399 7.48673 6.46116 6.43066 7.16797C5.39102 7.86387 4.57805 8.84864 4.09082 10C4.57802 11.1514 5.39102 12.1361 6.43066 12.832C7.4867 13.5388 8.72928 13.916 10 13.916C11.2707 13.916 12.5133 13.5388 13.5693 12.832C14.6089 12.1362 15.421 11.1513 15.9082 10C15.421 8.8488 14.6089 7.8638 13.5693 7.16797C12.5133 6.46116 11.2708 6.08399 10 6.08398ZM10 7.25C11.5188 7.25 12.75 8.48122 12.75 10C12.75 11.5188 11.5188 12.75 10 12.75C8.48122 12.75 7.25 11.5188 7.25 10C7.25 8.48122 8.48122 7.25 10 7.25ZM10 8.75C9.30964 8.75 8.75 9.30964 8.75 10C8.75 10.6904 9.30964 11.25 10 11.25C10.6904 11.25 11.25 10.6904 11.25 10C11.25 9.30964 10.6904 8.75 10 8.75ZM2.00977 2.00977H2V2H2.00977V2.00977Z" fill="#737373" /></svg>
                        ) : (
                    ...
                    {errors.repeatPassword && <p className="text-xs text-destructive mt-1">{errors.repeatPassword}</p>}
                  </div>
              ...
              <div>
                <label className="text-sm font-medium text-Stockia-Neutral-950 flex items-center gap-1">Rol asignado <span className="text-destructive">*</span></label>
                <Select className={cn(
                  "mt-2 bg-card",
              ...
              <div>
                <label className="text-sm font-medium text-Stockia-Neutral-950 flex items-center gap-1">Estado de la cuenta <span className="text-destructive">*</span></label>
                <Select className={
                  cn(
                    "mt-2 bg-card",
                    errors.accountStatus && "border-destructive focus-visible:ring-destructive",
                  )
                } value={form.accountStatus} onChange={(e) => setForm(f => ({ ...f, accountStatus: e.target.value }))}>
            ...
            <div className="mt-6 flex justify-end gap-4">
              <Button type="button" variant="ghost" className="w-36 h-11" onClick={handleClose}>Cancelar</Button>
              <Button type="submit" variant="stokia" className="w-44 h-11">{isEditMode ? 'Guardar cambios' : 'Registrar usuario'}</Button>
            </div>
          </form>
        )}

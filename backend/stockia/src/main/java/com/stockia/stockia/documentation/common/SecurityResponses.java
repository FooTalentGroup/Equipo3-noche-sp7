...
/**
 * Anotaciones reutilizables para respuestas de seguridad comunes en la
 * documentación API.
...
public class SecurityResponses {
  ...
  @ApiResponse(responseCode = "401", description = "No autorizado - Token ausente o inválido", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
      {
        "statusCode": 401,
  ...
  @Target({ ElementType.METHOD, ElementType.ANNOTATION_TYPE })
  @Retention(RetentionPolicy.RUNTIME)
  @ApiResponse(responseCode = "403", description = "Acceso denegado - Permisos insuficientes", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
      {
        "statusCode": 403,
        "errorCode": "FORBIDDEN",
  ...
  /**
   * Combinación de respuestas de seguridad para endpoints que requieren solo
   * ADMIN.
  ...
  /**
   * Combinación de respuestas de seguridad para endpoints que requieren ADMIN o
   * MANAGER.
   * Aplica automáticamente las respuestas 401 (No autorizado) y 403 (Acceso
   * denegado - ADMIN o MANAGER).
   */
  @Target(ElementType.ANNOTATION_TYPE)
  @Retention(RetentionPolicy.RUNTIME)
  @Unauthorized
